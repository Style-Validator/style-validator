...
//		that.deleteSaveButtonWhenNotLocal();
		if(STYLEV.isInChromeExtensionPage) {
			...
			that.asideBody.classList.add('is-hidden');
	...
	setParameters: function() {
		...
		that.asideBody = document.querySelector('.aside-body');
	...
	bindEvents: function() {
		...
		that.each(that.detailElements, function(detailElement) {
			detailElement.addEventListener('click', that.fixInScrolling);
		});
		...
		window.addEventListener('load', that.adjustScrollByHash);
	...
	*******************************************************************************************************************/
	...
	bindEvents2StyleListItem: function(styleList, styleListItem, isBaseStyle) {
		...
		that.each(formParts, function(formPart) {
			...
			formPart.addEventListener('blur', that.fixInScrolling);
	...
	fixInScrolling: function() {
		...
		that.each(that.fixInScrollingTargets, function(target) {
			var fixPositionHandler = that.fixPositionSetup(target);
			fixPositionHandler();
	...
	fixPositionSetup: function(target) {
		...
		if(isHorizontal) {
			ruleBoxStyleInner.style.setProperty('right', null, '');
		} else {
		...
		return function(event) {
			...
			that.each(fixTargets, function(fixTarget, i) {
				...
				if(targetGBCR.top <= that.mainHeaderOffsetHeight + 1) {//TODO: fix magic number
					...
					//transition
					if(targetGBCR.bottom <= fixTargetGBCR.bottom + 1) {
	...
	adjustScrollByHash: function(event) {
		...
		var that = STYLEV.RULES;
		if(location.hash && event.eventPhase === 2) {
			var target = document.querySelector(location.hash);
			var title = target.querySelector('.title').value + ' | ' + document.title;
			var targetPosY = target.offsetTop - that.mainHeaderOffsetHeight - 15;
			window.scrollTo(0, targetPosY);
			setTimeout(function() {
				window.scrollTo(0, targetPosY);
			}, 0);
			//TODO: implement pjax
			//history.pushState({hash: location.hash}, title, location.href);
		}
	}
};
